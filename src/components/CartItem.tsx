...
const CartItem = ({ id, imgUrl, title, count, price, type, size }: Props) => {
  ...
  const onClickRemove = () => {
    if (window.confirm('Чи дійсно Ви хочете видалити цей товар з корзини?')) {
    ...
    <>
      <li className="grid cart__item">
        ...
        <div className="cart__amount-wrapper">
          <button onClick={onClickMinus} className="cart__amount-btn">
            <IconAmountMinus />
          </button>
          <p className="cart__amount">{count}</p>
          <button onClick={onClickPlus} className="cart__amount-btn">
            <IconAmountPlus />
          </button>
        </div>
        <p className="cart__price">{price * count} грн.</p>
        <button onClick={onClickRemove} className="cart__remove-btn">
          <IconRemove />
        </button>
      </li>
    </>
